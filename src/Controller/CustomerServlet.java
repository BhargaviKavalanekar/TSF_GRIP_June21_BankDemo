...
public class CustomerServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action!=null && action.contentEquals("update"))
		{
			...
			custId=Integer.parseInt(request.getParameter("custId"));//cusromer1=sender
			...
			if(custAccBal>Trans_amt) //update sender
			{
				custAccBal=custAccBal-Trans_amt;
				Bank b=new Bank(custId,custName,custAccBal);
				flag=bd.updateAccount(b);
				
				if(flag) //update reciever
				{
				
					custAccNo =Integer.parseInt(request.getParameter("custAccNo2"));//customer2=reciever				
					...
					if(flag) //history
					{
						custId=Integer.parseInt(request.getParameter("custAccNo1")); //sender
						custAccNo =Integer.parseInt(request.getParameter("custAccNo2"));//reciever
						Trans_amt=Double.parseDouble(request.getParameter("Trans_amt"));
